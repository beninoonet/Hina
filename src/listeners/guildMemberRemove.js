  ...
  constructor(context, options) {
    ...
    async run(member) {
        ...
        // Generate a canvas
          ...
          ctx.fillText(`Au revoir !`, 240, 120);

          ctx.font = 'semi-bold 36px Montserrat';
          ctx.fillStyle = '#b9acff';
          ctx.fillText('@' + member.user.username, 240, 170);
              // Create attachment and send
          const attachment = new AttachmentBuilder(await canvas.encode('png'), { name: 'quit.png' });
        ...
        this.container.logger.info(`Member quit: ${member.user.tag}`);
